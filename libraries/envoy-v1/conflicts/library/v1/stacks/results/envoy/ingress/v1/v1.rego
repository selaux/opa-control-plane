package library.v1.stacks.results.envoy.ingress.v1
...
stack_allow {
	...
	data.stacks[stack_id].policy["com.styra.envoy.ingress"].rules.rules.allow
}

stack_deny {
	applicable_stacks[stack_id]
	data.stacks[stack_id].policy["com.styra.envoy.ingress"].rules.rules.allow == false
}

stack_status_codes[x] {
	applicable_stacks[stack_id]
	x := data.stacks[stack_id].policy["com.styra.envoy.ingress"].rules.rules.status_code
}
...
system_allow {
	data.policy["com.styra.envoy.ingress"].rules.rules.allow
}
...
system_status_code = x {
	x := data.policy["com.styra.envoy.ingress"].rules.rules.status_code
}

applicable_stacks[stack_id] {
	data.stacks[stack_id] = _  # TODO(tsandall): replace with data from builder if bottleneck found during testing
}

stacks_outcome[stack_id] = x {
	# stacks that set allow to either true or false
	applicable_stacks[stack_id]
	x := {"allowed": data.stacks[stack_id].policy["com.styra.envoy.ingress"].rules.rules.allow == true}
}

stacks_outcome[stack_id] = {} {
	# stacks that have allow as undefined
	applicable_stacks[stack_id]
	not data.stacks[stack_id].policy["com.styra.envoy.ingress"].rules.rules.allow == data.stacks[stack_id].policy["com.styra.envoy.ingress"].rules.rules.allow
}
...
headers[k] = v {
	data.policy["com.styra.envoy.ingress"].rules.rules.headers[k] = v
...
stacks_headers[k] = v {
	applicable_stacks[stack_id]
	data.stacks[stack_id].policy["com.styra.envoy.ingress"].rules.rules.headers[k] = v
}
