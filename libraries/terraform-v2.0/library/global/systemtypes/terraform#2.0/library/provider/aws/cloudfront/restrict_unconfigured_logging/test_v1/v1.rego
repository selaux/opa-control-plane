package global.systemtypes["terraform:2.0"].library.provider.aws.cloudfront.restrict_unconfigured_logging.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.aws.cloudfront.restrict_unconfigured_logging.v1

test_lgging_configured_good {
	...
	in := input_cloudfront_distribution(logging_configuration)
	actual := v1.prohibit_cloudfront_distribution_without_logging_configuration with input as in
	count(actual) == 0
}

test_logging_configured_empty_bad {
	logging_configuration := []
	in := input_cloudfront_distribution(logging_configuration)
	actual := v1.prohibit_cloudfront_distribution_without_logging_configuration with input as in
	count(actual) == 1
...
test_logging_configured_absent_bad {
	in := input_cloudfront_distribution_without_logging_configuration
	actual := v1.prohibit_cloudfront_distribution_without_logging_configuration with input as in
	count(actual) == 1
}

input_cloudfront_distribution_without_logging_configuration := x {
	x := {
		"configuration": {
			"provider_config": {"aws": {
				"expressions": {
...
input_cloudfront_distribution(logging_configuration) := x {
	x := {
		"configuration": {
			"provider_config": {"aws": {
				"expressions": {
