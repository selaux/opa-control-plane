package global.systemtypes["terraform:2.0"].library.provider.gcp.kics_libs.terraform
...
is_default_password(password) = output {
	contains(password, data.global.systemtypes["terraform:2.0"].library.provider.gcp.kics_libs.common_json.common_json.common_lib.default_passwords[_])
	output = true
} else = output {
