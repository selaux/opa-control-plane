package global.systemtypes["terraform:2.0"].library.provider.kubernetes.kics.no_drop_capabilities_for_containers.v2
...
no_drop_capabilities_for_containers_inner[result] {
...
no_drop_capabilities_for_containers_inner[result] {
	...
	result := {"documentId": input.document[i].id, "issueType": "IncorrectValue", "keyActualValue": sprintf("%s[%s].%s.%s.security_context.capabilities.drop is undefined", [resourceType, name, specInfo.path, types[x]]), "keyExpectedValue": sprintf("%s[%s].%s.%s.security_context.capabilities.drop should be set", [resourceType, name, specInfo.path, types[x]]), "resourceName": tf_lib.get_resource_name(resource, name), "resourceType": resourceType, "searchKey": sprintf("%s[%s].%s.%s.security_context.capabilities", [resourceType, name, specInfo.path, types[x]])}
}

# METADATA: library-snippet
# version: v1
# title: "KICS: No Drop Capabilities for Containers"
# description: >-
...
# resource-type: ""
# custom:
#   id: "kubernetes.kics.no_drop_capabilities_for_containers"
#   impact: ""
#   remediation: ""
#   severity: "low"
#   resource_category: ""
...
no_drop_capabilities_for_containers_snippet[violation] {
	no_drop_capabilities_for_containers_inner[inner] with input as data.global.systemtypes["terraform:2.0"].library.utils.v1.transformed_input
