package global.systemtypes["terraform:2.0"].library.provider.aws.ec2.without_encrypted_volume.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.aws.ec2.without_encrypted_volume.v1

input_ebs_block_device(encrypted) := x {
	x := [{
...
input_root_block_device(encrypted) := x {
	x := [{
		"delete_on_termination": true,
		"tags": null,
		"volume_size": 5,
...
test_instances_volume_deletion_good {
	...
	in := input_instance(ebs_value, root_value)
	actual := v1.ec2_without_encrypted_volume with input as in
	count(actual) == 0
}
...
input_instance(ebs_block_device, root_block_device) = x {
	x := {
		"format_version": "1.1",
