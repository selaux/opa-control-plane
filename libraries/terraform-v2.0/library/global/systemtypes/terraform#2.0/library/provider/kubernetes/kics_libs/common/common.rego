package global.systemtypes["terraform:2.0"].library.provider.kubernetes.kics_libs.common
...
get_module_equivalent_key(provider, moduleName, resource, key) = keyInResource {
	providers := data.global.systemtypes["terraform:2.0"].library.provider.kubernetes.kics_libs.common_json.common_json.common_lib.modules[provider]
...
is_aws_ebs_optimized_by_default(instanceType) {
	inArray(data.global.systemtypes["terraform:2.0"].library.provider.kubernetes.kics_libs.common_json.common_json.common_lib.aws_ebs_optimized_by_default, instanceType)
}
