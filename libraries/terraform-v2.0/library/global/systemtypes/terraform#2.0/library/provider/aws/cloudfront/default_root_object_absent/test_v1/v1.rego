package global.systemtypes["terraform:2.0"].library.provider.aws.cloudfront.default_root_object_absent.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.aws.cloudfront.default_root_object_absent.v1

test_default_root_object_absent_good {
	root_object := "index.html"
	in := input_cloudfront_distribution(root_object)
	actual := v1.prohibit_cloudfront_distribution_without_default_root_object with input as in
	count(actual) == 0
}

test_default_root_object_absent_bad {
	root_object := null
	in := input_cloudfront_distribution(root_object)
	actual := v1.prohibit_cloudfront_distribution_without_default_root_object with input as in
	count(actual) == 1
...
test_default_root_object_not_exists_bad {
	in := input_cloudfront_distribution_without_root_object
	actual := v1.prohibit_cloudfront_distribution_without_default_root_object with input as in
	count(actual) == 1
}

input_cloudfront_distribution(value) = x {
	x := {
		"format_version": "1.0",
					...
					"resources": [
						{
							...
							"type": "aws_cloudfront_distribution",
							"name": "s3_distribution",
							"provider_config_key": "sample_cloudfront_distribution:aws",
							"expressions": {
...
input_cloudfront_distribution_without_root_object = x {
	x := {
		"format_version": "1.0",
		"terraform_version": "1.1.9",
		"planned_values": {"root_module": {"child_modules": [{
