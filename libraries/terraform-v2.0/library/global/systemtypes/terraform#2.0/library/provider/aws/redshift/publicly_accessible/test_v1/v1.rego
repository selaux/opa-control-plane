package global.systemtypes["terraform:2.0"].library.provider.aws.redshift.publicly_accessible.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.aws.redshift.publicly_accessible.v1

test_publicly_accessible_redshift_good {
	accessible := false
	in := input_redshift_cluster(accessible)
	actual := v1.prohibit_publicly_accessible_redshift_cluster with input as in
	count(actual) == 0
}

test_publicly_accessible_redshift_bad {
	accessible := true
	in := input_redshift_cluster(accessible)
	actual := v1.prohibit_publicly_accessible_redshift_cluster with input as in
	count(actual) == 1
...
test_publicly_accessible_not_exists_in_redshift_bad {
	in := input_redshift_cluster_without_publicly_accessible
	actual := v1.prohibit_publicly_accessible_redshift_cluster with input as in
	count(actual) == 1
}
...
input_redshift_cluster_without_publicly_accessible = x {
	x := {
		"format_version": "1.1",
		"terraform_version": "1.3.6",
		"variables": {"region": {"value": "us-west-2"}},
