package global.systemtypes["terraform:2.0"].library.provider.aws.elb.restrict_without_connection_draining.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.aws.elb.restrict_without_connection_draining.v1

test_elb_connection_draining_good {
	connection_draining := true
	in := input_elastic_loadbalancer(connection_draining)
	actual := v1.prohibit_elastic_load_balancer_without_connection_draining_set_to_true with input as in
	count(actual) == 0
}

test_elb_connection_draining_bad {
...
test_elb_connection_draining_absent {
	in := input_elastic_loadbalancer_without_connection_draining
	actual := v1.prohibit_elastic_load_balancer_without_connection_draining_set_to_true with input as in
	count(actual) == 1
}

input_elastic_loadbalancer(connection_draining) := x {
	x := {
		"format_version": "1.1",
...
input_elastic_loadbalancer_without_connection_draining := x {
	x := {
		"format_version": "1.1",
		"terraform_version": "1.2.7",
		"planned_values": {"root_module": {"child_modules": [{
