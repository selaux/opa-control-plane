package global.systemtypes["terraform:2.0"].library.provider.azure.postgres_db.backup_disabled.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.azure.postgres_db.backup_disabled.v1

test_prohibit_backup_disabled_postgres_db_server_good {
	db := input_db(true)
	actual := v1.prohibit_backup_disabled_postgres_db_server with input as db

	count(actual) == 0
}

test_prohibit_backup_disabled_postgres_db_server_bad {
	db := input_db(false)
	actual := v1.prohibit_backup_disabled_postgres_db_server with input as db
...
test_prohibit_backup_disabled_postgres_db_server_undefined {
		...
		"planned_values": {"root_module": {"resources": [
			{
				...
				"values": {
					...
					"server_name": "postgresql-server-1",
					"timeouts": null,
				},
			},
			{
			...
			"root_module": {"resources": [
				{
					...
					"expressions": {
						"charset": {"constant_value": "UTF8"},
						"collation": {"constant_value": "English_United States.1252"},
						"name": {"constant_value": "exampledb"},
						"resource_group_name": {"references": ["azurerm_resource_group.example"]},
						"server_name": {"references": ["azurerm_postgresql_server.example"]},
					},
					"schema_version": 0,
				},
				{
					...
					"provider_config_key": "azurerm",
					"expressions": {
						"administrator_login": {"constant_value": "psqladminun"},
