package global.systemtypes["terraform:2.0"].library.provider.aws.opensearch.node_to_node_encryption.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.aws.opensearch.node_to_node_encryption.v1

test_node_to_node_encryption_opensearch_good {
	enabled := true
	in := input_opensearch_domain(enabled)
	actual := v1.prohibit_opensearch_domains_with_disabled_node_to_node_encryption with input as in

	count(actual) == 0
}

test_node_to_node_encryption_opensearch_bad {
	enabled := false
	in := input_opensearch_domain(enabled)
	actual := v1.prohibit_opensearch_domains_with_disabled_node_to_node_encryption with input as in
...
test_node_to_node_encryption_opensearch_not_configured {
