package global.systemtypes["terraform:2.0"].library.provider.azure.mariadb.backup_disabled.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.azure.mariadb.backup_disabled.v1

test_prohibit_backup_disabled_mariadb_db_server_good {
	db := input_db(true)
	actual := v1.prohibit_backup_disabled_mariadb_db_server with input as db

	count(actual) == 0
}

test_prohibit_backup_disabled_mariadb_db_server_bad {
	db := input_db(false)
	actual := v1.prohibit_backup_disabled_mariadb_db_server with input as db
...
test_prohibit_backup_disabled_mariadb_db_server_undefined {
...
input_db(value) = x {
	x := {
		...
		"planned_values": {"root_module": {"resources": [
			{
				"address": "azurerm_mariadb_server.example",
				"mode": "managed",
			...
			"root_module": {"resources": [
				{
					...
					"expressions": {
						...
						"name": {"constant_value": "example-mariadb-server"},
