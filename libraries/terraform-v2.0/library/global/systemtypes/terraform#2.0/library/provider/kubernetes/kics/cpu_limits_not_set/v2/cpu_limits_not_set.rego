package global.systemtypes["terraform:2.0"].library.provider.kubernetes.kics.cpu_limits_not_set.v2
...
cpu_limits_not_set_inner[result] {
	resource := input.document[i].resource[resourceType]
	specInfo := tf_lib.getSpecInfo(resource[name])
	containers := specInfo.spec[types[x]]
	is_array(containers) == true
	containerTypes := containers[_0]
	not common_lib.valid_key(containerTypes.resources.limits, "cpu")
	result := {"documentId": input.document[i].id, "issueType": "MissingAttribute", "keyActualValue": sprintf("%s[%s].%s.%s[%d].resources.limits.cpu is undefined", [resourceType, name, specInfo.path, types[x], containerTypes]), "keyExpectedValue": sprintf("%s[%s].%s.%s[%d].resources.limits.cpu should be set", [resourceType, name, specInfo.path, types[x], containerTypes]), "resourceName": tf_lib.get_resource_name(resource, name), "resourceType": resourceType, "searchKey": sprintf("%s[%s].%s.%s", [resourceType, name, specInfo.path, types[x]])}
}
...
cpu_limits_not_set_inner[result] {
	...
	result := {"documentId": input.document[i].id, "issueType": "MissingAttribute", "keyActualValue": sprintf("%s[%s].%s.%s.resources.limits.cpu is undefined", [resourceType, name, specInfo.path, types[x]]), "keyExpectedValue": sprintf("%s[%s].%s.%s.resources.limits.cpu should be set", [resourceType, name, specInfo.path, types[x]]), "resourceName": tf_lib.get_resource_name(resource, name), "resourceType": resourceType, "searchKey": sprintf("%s[%s].%s.%s.resources.limits", [resourceType, name, specInfo.path, types[x]])}
}
...
# resource-type: ""
# custom:
#   id: "kubernetes.kics.cpu_limits_not_set"
#   impact: ""
#   remediation: ""
#   severity: "medium"
#   resource_category: ""
...
cpu_limits_not_set_snippet[violation] {
	cpu_limits_not_set_inner[inner] with input as data.global.systemtypes["terraform:2.0"].library.utils.v1.transformed_input
