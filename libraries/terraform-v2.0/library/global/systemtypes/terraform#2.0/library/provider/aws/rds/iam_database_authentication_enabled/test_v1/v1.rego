package global.systemtypes["terraform:2.0"].library.provider.aws.rds.iam_database_authentication_enabled.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.aws.rds.iam_database_authentication_enabled.v1

test_iam_database_authentication_rds_good {
	accessible := true
	in := input_rds_instance(accessible)
	actual := v1.prohibit_rds_instance_with_disabled_iam_authentication with input as in
	count(actual) == 0
}

test_iam_database_authentication_rds_bad {
	accessible := false
	in := input_rds_instance(accessible)
	actual := v1.prohibit_rds_instance_with_disabled_iam_authentication with input as in
	count(actual) == 1
...
test_iam_database_authentication_rds_not_configured {
	accessible := null
	in := input_rds_instance(accessible)
	actual := v1.prohibit_rds_instance_with_disabled_iam_authentication with input as in
	count(actual) == 1
...
test_iam_database_authentication_rds_instance_not_present {
	in := input_rds_instance_missing_setting
	actual := v1.prohibit_rds_instance_with_disabled_iam_authentication with input as in
	count(actual) == 1
}

input_rds_instance_missing_setting := x {
	x := {
		"format_version": "1.1",
