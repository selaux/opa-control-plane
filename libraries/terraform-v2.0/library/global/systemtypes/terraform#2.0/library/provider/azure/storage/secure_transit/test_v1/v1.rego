package global.systemtypes["terraform:2.0"].library.provider.azure.storage.secure_transit.test_v1

import data.global.systemtypes["terraform:2.0"].library.provider.azure.storage.secure_transit.v1

test_restrict_storage_account_unecrypted_transit_good {
	tf := input_with_storage_account_transit(true)
	actual := v1.restrict_storage_account_unecrypted_transit with input as tf

	count(actual) == 0
}

test_restrict_storage_account_unecrypted_transit_bad {
	tf := input_with_storage_account_transit(false)
	actual := v1.restrict_storage_account_unecrypted_transit with input as tf

	count(actual) == 1
}

input_with_storage_account_transit(value) = x {
	x := {
		"format_version": "0.1",
		...
		"configuration": {
			...
			"root_module": {"resources": [
				...
				},
				{
					...
					"provider_config_key": "azurerm",
					"expressions": {
						"account_replication_type": {"constant_value": "GRS"},
						...
						"name": {"constant_value": "storageaccountname"},
						"resource_group_name": {"references": ["azurerm_resource_group.example"]},
						"tags": {"constant_value": {"environment": "staging"}},
					},
					"schema_version": 2,
				},
			]},
